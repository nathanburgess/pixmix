...
type varType = 
    | Num 
    | Int (* This is needed JUST to make a main for LLVM *)
...
let rec string_of_expr = function
    ...
    | ArrOp(s, e) -> s ^ "[" ^ string_of_expr e ^ "]"
    | ObjLit(s1, s2) -> s1 ^ "." ^ s2 
    | ObjCall(s1, s2, e) -> s1 ^ "." ^ s2 ^ "(" ^ String.concat ", " (List.map string_of_expr e) ^ ")"
...
let string_of_program globals =
    String.concat "" (List.map string_of_fdecl globals.functions) ^ "\n" ^
