...
let getNullForType = function
    ...
    | _ -> raise (Failure "[Error] Type Not Found for getNullForType.")
...
let translate program =
    let functionDecls =
        ...
        in List.fold_left funDecl StringMap.empty program in
            ...
            let addLocal m =
                function
                    | S.Formal (t, vName) ->
                        let vName' = getVariableName fdecl.S.name vName in
                        let local_var =
                            L.define_global vName' (getDefaultValue t) the_module
                        in StringMap.add vName' (local_var, t) m in
            let formals =
                List.fold_left2 addFormal StringMap.empty fdecl.S.args
                    (Array.to_list (L.params func))
            in add_to_context (List.fold_left addLocal formals fdecl.S.locals) in

        let lookup vName =
            let getFunctionParentName fName =
                let (_, fdecl) = StringMap.find fName functionDecls
                in fdecl.S.parent in
            let rec aux vName fName =
                try
                    StringMap.find (getVariableName fName vName)
                        (Hashtbl.find context_funcs_vars fName)
                with
                    | Not_found ->
                        if fName = "main"
                        then raise (Failure "[Error] Local Variable not found.")
                        else aux vName (getFunctionParentName fName)
            in aux vName fdecl.S.name in
